...
try:
    ...
    print(f"ERRORE: Impossibile connettersi al DB. {e}", file=sys.stderr)
...
def _get_active_ingredients_by_aic(codice_aic: int) -> list[PrincipioAttivo]:
    ...
    # model_validate() controlla che i dati siano validi e crea una istanza di PrincipioAttivo per ogni riga
...
def _analizza_stato_farmaco_per_nome(nome_commerciale: str) -> StatoCommercialeFarmaco | None:
    ...
    # I caso: non ci sono confezioni acquistabili
...
# ctx viene passato automaticamente da MCP per fini di logging
