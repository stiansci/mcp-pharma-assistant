...
from io import BytesIO # "traveste" i nostri byte in memoria facendoli sembrare un file vero per etree di lxml
...
OUTPUT_DIR: str = os.path.join(base_dir, "parquet_files")
    ...
    try:
        ...
        for _, elem in context:
            ...
            if db_id and name:
                ...
                d_type = elem.get('type', None) 
                d_state = elem.findtext('db:state', namespaces=ns) 

                smiles_list = elem.xpath(
                    "db:calculated-properties/db:property[db:kind='SMILES']/db:value/text()",
                    namespaces=ns
                )
                d_smiles = smiles_list[0] if smiles_list else None
                # ------------------------------

                # includiamo d_smiles alla fine della tupla
                data["drugs"].append((db_id, name, d_type, d_state, d_smiles))
